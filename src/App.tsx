    ...
    <div>
      ...
      <div className='flex w-full h-screen justify-center items-center'>
        <Form className='flex flex-col gap-3 items-start text-lg p-16 rounded-md border-2 border-solid border-primary dark:border-secondary dark:text-secondary' onSubmit={({values})=> console.log(values)} yupSchema={schema} >
          <Input label='Email' name='email' type='email' placeholder='example@.com' required className='rounded'/>
          <Input label='Password' name='password' type='password' placeholder='********' required className='rounded'/>
          <button type='submit' className='bg-primary rounded-md text-secondary dark:bg-secondary dark:text-primary px-4 mt-2'>Submit</button>
          {/* <Debug/> */}
        </Form>
      </div>
    </div>
